...
export function upsertStagingAssetRow(
  ...
  const mappingStatus = view.needsReview ? 'PENDING_REVIEW' : 'AUTO_MAPPED';
  const needsReview = view.needsReview ? 1 : 0;
  ...
  const params = {
    ...
    // proposed_* 컬럼에는 fallback이 아닌 "확정값"만 채운다. fallback인 경우 NULL로
    // 남겨 PENDING_REVIEW 큐에서 "미확정"임이 명확히 드러나도록 한다.
    proposedEquipmentTag: view.fallbackFields.includes('equipmentTag') ? null : view.equipmentTag,
    proposedKksCode: view.fallbackFields.includes('kksCode') ? null : view.kksCode,
