...
//   9개 PatrolDomain 중 8개(ISO Tank Cargo 제외, 아래 참고)로 확장한다.
//   태그 값은 patrolEquipmentTags.ts(순수 Logic/Data Layer 레지스트리,
//   Stage C1 리팩터로 B1 컴포넌트 파일에서 승격됨)에서 가져온다.
//
//   Stage E-1: ng_buffer_tank(V-101)가 patrolEquipmentTags.ts의
//   PATROL_EQUIPMENT_TAGS_BY_DOMAIN에 등록되는 즉시 CANDIDATE_TAG_DOMAIN이
//   flatMap으로 자동 반영한다 — 이 파일에서 별도 등록 코드가 필요 없다.
//
...
export const PRIMARY_COLUMN_BY_DOMAIN: Partial<Record<PatrolDomain, string>> = {
  ...
  ng_buffer_tank: 'pressure_transmitter_barg',
};
