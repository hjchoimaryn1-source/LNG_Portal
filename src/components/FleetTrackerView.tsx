...
export default function FleetTrackerView() {
  ...
  return (
    <div className="flex flex-col gap-5 sm:gap-6 w-full">
      {/* Toast Notification */}
      {successToast && (
        <div className="fixed top-20 right-4 sm:right-8 z-50 flex items-center gap-2 px-4 py-3 bg-emerald-500/20 border border-emerald-500/50 text-emerald-300 rounded-xl shadow-2xl backdrop-blur-md animate-in fade-in slide-in-from-top-4">
      ...
      <section>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
          ...
          <div className="text-xs font-mono text-slate-400 bg-slate-900/80 px-3 py-1.5 rounded-lg border border-slate-800 self-start sm:self-auto shrink-0">
          ...
          {(Object.keys(NODE_CONFIGS) as NodeState[]).map((nodeKey) => {
            ...
            return (
              <div
                ...
                className={`p-3.5 sm:p-4 rounded-xl border cursor-pointer transition-all duration-200 relative overflow-hidden group ${
                  isSelected
                    ? `${config.borderColor} ${config.bgColor} shadow-lg shadow-blue-500/10 ring-1 ring-blue-500/50`
              ...
              >
                ...
                <p className="text-[10px] sm:text-[11px] text-slate-400 truncate mb-2">{config.subTitle}</p>
                {/* Progress bar */}
      ...
      <section className="bg-slate-900/80 border border-slate-800/80 rounded-xl p-3.5 sm:p-4 flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-3 sm:gap-4 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
          <div className="relative flex-1 sm:flex-initial sm:min-w-[220px]">
            ...
            <input
              ...
              className="w-full pl-8 sm:pl-9 pr-3 py-1.5 text-xs bg-slate-950 border border-slate-800 rounded-lg text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
            />
          </div>

          <div className="flex flex-wrap items-center gap-1 bg-slate-950 p-1 rounded-lg border border-slate-800 text-xs">
            <Filter className="w-3 h-3 text-slate-400 ml-1 mr-0.5 shrink-0 hidden xs:inline" />
            <button
              onClick={() => setSelectedNodeFilter('ALL')}
              className={`px-2 py-0.8 sm:px-2.5 sm:py-1 rounded-md transition-colors text-[11px] sm:text-xs ${
            ...
            {(Object.keys(NODE_CONFIGS) as NodeState[]).map((nk) => (
              <button
                ...
                className={`px-1.5 sm:px-2 py-0.8 sm:py-1 rounded-md transition-colors text-[11px] sm:text-xs whitespace-nowrap ${
        ...
        <div className="flex flex-wrap items-center justify-between sm:justify-end gap-2.5 sm:gap-3 pt-2 xl:pt-0 border-t xl:border-t-0 border-slate-800">
          <button
            onClick={selectAllFiltered}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 rounded-lg text-xs text-slate-300 font-medium transition-colors shrink-0"
          >
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 hidden sm:inline">Move:</span>
            <select
              value={transitionTargetNode}
              onChange={(e) => setTransitionTargetNode(e.target.value as NodeState)}
              className="bg-slate-950 border border-slate-800 text-slate-200 text-xs rounded-lg px-2 sm:px-2.5 py-1.5 focus:outline-none focus:border-blue-500 max-w-[170px] sm:max-w-none"
            >
            ...
            <button
              ...
              className={`flex items-center gap-1 sm:gap-1.5 px-3 sm:px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all shrink-0 ${
      ...
      <section>
          ...
          {filteredTanks.map((tank) => {
            ...
            return (
              <div
                ...
                className={`p-3 sm:p-3.5 rounded-xl border cursor-pointer transition-all duration-200 relative group flex flex-col justify-between ${
                  isSelected
                    ? 'bg-blue-950/35 border-blue-500 shadow-md shadow-blue-500/10 ring-1 ring-blue-500/40'
              ...
              >
                <div>
                  ...
                  <div className="text-[10px] text-slate-400 font-mono truncate mb-2">{tank.serialNo}</div>

                  <div className={`text-[10px] font-semibold px-2 py-0.5 rounded-md border inline-flex items-center gap-1 mb-2.5 max-w-full truncate ${nodeConfig.badgeBg}`}>
                    {nodeConfig.icon}
                ...
                <div className="mt-2.5 pt-1.5 border-t border-slate-800/40 flex items-center justify-between text-[10px] text-slate-500">
                  ...
                  <span className="font-mono text-slate-400 truncate max-w-[85px]">{tank.position}</span>
                </div>
              </div>
            );
          })}
        ...
        {filteredTanks.length === 0 && (
          <div className="text-center py-16 bg-slate-900/30 border border-slate-800 rounded-xl text-slate-500 text-sm">
