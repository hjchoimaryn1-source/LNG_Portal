// src/components/locations/nias/tabs/NiasLaydownHeelTab.tsx
"use client";
...
export interface NiasLaydownHeelTabProps {
  zoneStats: {
    yard2: {
      tanks: NiasTankAsset[];
      avgPress: number;
  ...
  ld2ViewMode?: 'STAGING_BUFFER' | 'SHIPPING_REPORT';
  setLd2ViewMode?: (mode: 'STAGING_BUFFER' | 'SHIPPING_REPORT') => void;
  dragOverTarget: string | null;
  setDragOverTarget: (target: string | null) => void;
  draggingTankNo: string | null;
  setDraggingTankNo: (tankNo: string | null) => void;
  setToastMessage: (msg: string | null) => void;
  handleOpenLd2VentModal: (tank: NiasTankAsset) => void;
  handleExportShippingReport: () => void;
}

export default function NiasLaydownHeelTab({
  zoneStats,
  selectedBackhaulTanks,
  setSelectedBackhaulTanks,
  handleAuthorizeBackhaul,
  ld2ViewMode: propLd2ViewMode,
  setLd2ViewMode: propSetLd2ViewMode,
  dragOverTarget,
  setDragOverTarget,
  draggingTankNo,
  setDraggingTankNo,
...
}: NiasLaydownHeelTabProps) {
  const [internalViewMode, setInternalViewMode] = useState<'STAGING_BUFFER' | 'SHIPPING_REPORT'>('STAGING_BUFFER');
  const activeViewMode = propLd2ViewMode ?? internalViewMode;
  const setViewMode = propSetLd2ViewMode ?? setInternalViewMode;
  ...
  const totalHeelKg = yard2TanksList.reduce(
    (acc, t) => acc + Math.round(((t.levelPercent || 4.0) / 100) * 18200),
    0
  );
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-200 font-mono">
      ...
      <div className="flex items-center gap-1 border-b-2 border-[#1e293b] pb-0 px-1 pt-1 bg-[#dfdbd1] rounded-xs select-none">
        <button
          type="button"
          onClick={() => setViewMode('STAGING_BUFFER')}
        ...
        <button
          type="button"
          onClick={() => setViewMode('SHIPPING_REPORT')}
          className={`px-4 py-2 font-mono text-xs font-black tracking-wide border-t-2 border-l-2 border-r-2 rounded-t-xs transition-all cursor-pointer flex items-center gap-1.5 ${
            activeViewMode === 'SHIPPING_REPORT'
              ? 'bg-[#ece9d8] text-[#002b4d] border-t-white border-l-white border-r-slate-600 shadow-xs -mb-[2px] pb-2.5 z-10'
              : 'bg-[#d0cbbf] hover:bg-[#dedad0] text-slate-700 border-t-slate-300 border-l-slate-300 border-r-slate-500'
          }`}
        >
      ...
      {activeViewMode === 'STAGING_BUFFER' && (
        <div className="space-y-3.5 animate-in fade-in duration-150">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
            ...
            <div className="bg-[#e8e4dc] border border-[#b0aaa0] rounded-xs p-2.5 shadow-xs flex flex-col justify-between">
              <span className="text-[10px] font-bold text-slate-600 uppercase tracking-tighter">
                AVG HOLDING PRESSURE
              </span>
              <div className="flex items-baseline gap-1.5 my-0.5">
                <span className="font-mono text-xl font-black text-slate-900">
                  {zoneStats.yard2.avgPress.toFixed(2)}
                </span>
                <span className="text-xs font-bold text-slate-600">MPa</span>
              </div>
              <span className="text-[9px] text-slate-500 font-bold truncate">
                Safe marine transit margin
              </span>
            </div>
                        ...
                        <div className="relative w-full h-[64px] flex items-center justify-center bg-slate-200/70 rounded-xs border border-slate-300/90 p-1 shadow-inner my-0.5 pointer-events-none">
                          <svg viewBox="0 0 420 86" className="w-full h-full" preserveAspectRatio="none">
                            ...
                            <g clipPath={`url(#innerWindowClip-ld2-buf-${tank.id})`}>
                              ...
                              <text x="70" y="25" fill="#475569" fontSize="8" fontWeight="bold" fontFamily="monospace" letterSpacing="0.8">
                                GAS / VAPOR (BOG)
                              </text>
                              <text x="350" y="25" textAnchor="end" fill="#64748b" fontSize="7.5" fontWeight="bold" fontFamily="monospace" letterSpacing="0.5">
                                HEADSPACE
                              </text>
