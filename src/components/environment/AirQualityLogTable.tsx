...
//
// Phase 11b Stage 2-C — pure display component for env_air_quality_logs.
// Props-in only; no DAO/context calls. status is rendered verbatim as stored
// (including 'PENDING_REVIEW') — no PASS/FAIL inference happens here.

'use client';

import type { AirQualityLog } from '../../cmms-environment/types/environment';
...
interface AirQualityLogTableProps {
  records: AirQualityLog[];
}

export default function AirQualityLogTable({ records }: AirQualityLogTableProps) {
  return (
      ...
      <table className="w-full text-left border-collapse font-mono text-[11px] win-grid">
        <thead>
          <tr className="bg-slate-200 border-b border-slate-400">
            ...
            <th className="p-1.5 border-r border-slate-300 text-right">SO2 (µg/m³)</th>
            <th className="p-1.5 border-r border-slate-300 text-right">NOx (µg/m³)</th>
            <th className="p-1.5 border-r border-slate-300 text-right">CO (µg/m³)</th>
            <th className="p-1.5 border-r border-slate-300 text-right">PM10 (µg/m³)</th>
        ...
        <tbody>
          ...
          {records.map((r, i) => (
            <tr key={r.id} className={`border-b border-slate-200 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
              ...
              <td className="p-1.5 border-r border-slate-300 text-right">{r.so2Ugm3 ?? '—'}</td>
              <td className="p-1.5 border-r border-slate-300 text-right">{r.noxUgm3 ?? '—'}</td>
              <td className="p-1.5 border-r border-slate-300 text-right">{r.coUgm3 ?? '—'}</td>
              <td className="p-1.5 border-r border-slate-300 text-right">{r.pm10Ugm3 ?? '—'}</td>
