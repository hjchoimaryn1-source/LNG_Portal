...
//   (SqlExecutor only), same pattern as gasMeteringLedgerDao.ts.
...
export function upsertIsoTankDailyReading(db: SqlExecutor, row: IsoTankDailyReadingRow): void {
  db.run(
    `INSERT INTO iso_tank_daily_readings (
       report_date, iso_tank_no, serial_no, shipment, level_pct, level_m3,
       level_mmh2o, battery_pct, pressure_mpa, temp_c, depress_flag,
       press_before_mpa, press_after_mpa, remarks
     ) VALUES (
       @reportDate, @isoTankNo, @serialNo, @shipment, @levelPct, @levelM3,
       @levelMmh2o, @batteryPct, @pressureMpa, @tempC, @depressFlag,
       @pressBeforeMpa, @pressAfterMpa, @remarks
     )
     ON CONFLICT(report_date, iso_tank_no) DO UPDATE SET
       ...
       press_after_mpa = excluded.press_after_mpa, remarks = excluded.remarks`,
    { ...row }
  );
}
