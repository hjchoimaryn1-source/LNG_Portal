...
foreach ($file in $files) {
    if (Test-Path $file) {
        ...
        # Button fixes
        $content = $content -replace '(bg-blue-600[^''"]*?)text-slate-[0-9]+', '$1text-white'
        $content = $content -replace '(bg-emerald-600[^''"]*?)text-slate-[0-9]+', '$1text-white'
        $content = $content -replace '(bg-amber-600[^''"]*?)text-slate-[0-9]+', '$1text-white'
        $content = $content -replace '(bg-blue-500[^''"]*?)text-slate-[0-9]+', '$1text-white'

        $content = $content -replace '(bg-emerald-600[^''"]*?)text-slate-[0-9]+ font-bold', '$1text-white font-bold'
        $content = $content -replace '(bg-amber-600[^''"]*?)text-slate-[0-9]+ font-bold', '$1text-white font-bold'
        $content = $content -replace '(bg-blue-600[^''"]*?)text-slate-[0-9]+ font-bold', '$1text-white font-bold'
        
        # Save explicitly with UTF-8 no BOM
